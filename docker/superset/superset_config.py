...
# Theme — primary-only override so buttons / links / chart accents pick up
# MonCTL's brand-blue. Also tried overriding `grayscale` (body bg + surfaces)
# but Superset 4.1's chrome (top nav, cards) doesn't read those tokens — only
# the body did, leaving an ugly white-chrome / dark-body band. Native dark
# mode lands in Superset 5.0; until then, primary-only is the safe middle.
THEME_OVERRIDES = {
    "colors": {
        "primary": {"base": "#0134a8"},    # MonCTL brand-500
        "secondary": {"base": "#00217c"},  # brand-600
    },
    "borderRadius": 6,
}
