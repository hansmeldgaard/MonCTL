...
| `MONCTL_REQUIRE_PER_COLLECTOR_AUTH` | `false` | no        | no                | When `true`, central rejects shared-secret callers that can't be matched to a specific collector. Default off during the rollout because the customer installer doesn't yet provision per-collector keys.                |
