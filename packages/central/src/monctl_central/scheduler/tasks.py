...
class SchedulerRunner:
    ...
    async def _sample_ingestion_rates(self) -> None:
        ...
        for each of the core ingestion tables, divides by the window, and
        writes one row per table into `ingestion_rate_history`. Runs on
        the leader only every 5 min. Failures are logged, never block the
        scheduler.
        """
