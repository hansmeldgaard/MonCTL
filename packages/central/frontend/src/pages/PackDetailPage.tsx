    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          {pack.author && (
            <p className="mt-0.5 text-xs text-zinc-500">by {pack.author}</p>
          )}
        </div>
                  ...
                  {isOpen && count > 0 && (
                    <div className="ml-8 mb-2">
                      {manifest.map((name: string) => (
                        <div
                          key={name}
                          className="py-0.5 text-sm text-zinc-400"
                        >
                          {name}
                        </div>
                      ))}
                      {manifest.length === 0 && (
                        <div className="py-0.5 text-xs text-zinc-600">
                          {count} entities (no manifest details)
                        </div>
                      )}
                    </div>
                  )}
                  {isOpen && count === 0 && (
                    <div className="ml-8 mb-2 py-0.5 text-xs text-zinc-600">
                      None
                    </div>
                  )}
                </div>
                ...
                <TableBody>
                  {pack.versions.map((v) => (
                    <TableRow key={v.id}>
                      ...
                      <TableCell className="text-zinc-400 max-w-[400px] truncate">
                        {v.changelog ?? "—"}
                      </TableCell>
                      <TableCell className="text-zinc-500">
                        {formatDate(v.imported_at, tz)}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
      ...
      >
        <p className="text-sm text-zinc-400">
          Delete pack{" "}
          <span className="font-semibold text-zinc-200">{pack.name}</span>?
        </p>
