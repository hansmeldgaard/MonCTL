...
import { ArrowLeft, AppWindow, Code2, Loader2, Pencil, Plus } from "lucide-react";
...
export function AppDetailPage() {
  ...
  async function handleEditApp(e: React.FormEvent) {
    ...
    setEditError(null);
    try {
      await updateApp.mutateAsync({ id, data: { description: editDesc.trim() } });
      setEditOpen(false);
    } catch (err) {
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-3">
          ...
          <Badge variant={app.app_type === "builtin" ? "default" : "info"}>{app.app_type}</Badge>
          <button
            onClick={() => { setEditDesc(app.description ?? ""); setEditError(null); setEditOpen(true); }}
      ...
      <Dialog open={editOpen} onClose={() => setEditOpen(false)} title="Edit App">
        <form onSubmit={handleEditApp} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input id="app-edit-desc" value={editDesc} onChange={(e) => setEditDesc(e.target.value)} autoFocus />
          </div>
      ...
      {/* New Version Dialog */}
      <Dialog open={versionOpen} onClose={() => setVersionOpen(false)} title="New Version">
        <form onSubmit={handleCreateVersion} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <CodeEditor value={versionCode} onChange={setVersionCode} height="250px" />
          </div>
          <div className="space-y-1.5">
      ...
      {/* View Version Dialog */}
      <Dialog open={!!viewVersion} onClose={() => setViewVersion(null)} title={`Version ${viewVersion?.version ?? ""}`}>
        {viewVersion && (
          <div className="space-y-4">
            ...
            {viewVersion.source_code && (
              <div>
                ...
                <CodeEditor value={viewVersion.source_code} readOnly height="300px" />
              </div>
            )}
          </div>
        )}
