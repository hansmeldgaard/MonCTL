...
const installJobStatusConfig: Record<
  string,
  { color: string; icon: typeof CheckCircle2 }
> = {
  pending: {
...
export function UpgradesPage() {
  ...
  const [expandedInstallJobId, setExpandedInstallJobId] = useState<
    string | null
  >(null);
  ...
  const nodes = useMemo(() => {
    ...
    if (nodeFilterGroup)
      filtered = filtered.filter((n) =>
        (n.group_name || "")
          .toLowerCase()
          .includes(nodeFilterGroup.toLowerCase()),
      );
    if (nodeFilterIp)
  ...
  const installPlan = useMemo(() => {
    ...
    const rebootPkgs = pkgItems
      .filter((p) => p.requires_reboot)
  ...
  const rebootableSelectedNodes = useMemo(() => {
    return nodes.filter(
      (n) => selectedNodes.has(n.hostname) && n.reboot_required,
    );
  }, [nodes, selectedNodes]);
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-400">
            Manage system versions and orchestrate rolling upgrades
          </p>
        </div>
      </div>
        ...
        <CardHeader className="pb-3">
          ...
          {restartNodes.isError && (
            <div className="mt-2 rounded-md bg-red-500/10 border border-red-500/30 p-2 text-xs text-red-400">
              ...
              {(restartNodes.error as Error)?.message ??
                "Failed to restart nodes"}
            </div>
          )}
        </CardHeader>
        <CardContent>
      ...
      <Card className="border-zinc-800 bg-zinc-900">
        ...
        <CardContent>
          {jobsLoading ? (
            ...
            <p className="text-sm text-zinc-500 py-4 text-center">
              No upgrade jobs have been created yet.
            </p>
          ) : (
        ...
        <CardContent>
          {checkOs.isError && (
            <div className="mb-3 rounded-md bg-red-500/10 border border-red-500/30 p-3 text-sm text-red-400">
              <AlertTriangle className="inline h-4 w-4 mr-1" />
              Check failed:{" "}
              {(checkOs.error as Error)?.message ?? "Unknown error"}
            </div>
          )}
          ...
          {selectedPkgs.size > 0 && (
            <div className="mb-3 flex items-center gap-3 rounded-lg border border-brand-500/30 bg-brand-500/5 px-4 py-2.5">
              <span className="text-sm text-zinc-300 font-medium">
                {selectedPkgs.size} package{selectedPkgs.size !== 1 ? "s" : ""}{" "}
                selected
              </span>
              <div className="flex-1" />
              <Button size="sm" onClick={() => setShowInstallPreview(true)}>
          ...
          {(() => {
            const filtered = (packageInventory ?? []).filter(
              (p) =>
                !pkgSearch ||
                p.package_name.toLowerCase().includes(pkgSearch.toLowerCase()),
            );
            const allFilteredSelected =
              filtered.length > 0 &&
              filtered.every((p) => selectedPkgs.has(p.package_name));
            return (
              <>
                {filtered.length === 0 ? (
                  <p className="text-sm text-zinc-500 py-4 text-center">
                      ...
                      <TableBody>
                        {filtered.map((pkg) => {
                          ...
                          const pct =
                            pkg.total_nodes > 0
                              ? Math.round(
                                  (pkg.installed_count / pkg.total_nodes) * 100,
                                )
                              : 0;
                          return (
                            <>
                              <TableRow
                                ...
                                onClick={() =>
                                  setExpandedPkg(
                                    isExpanded ? null : pkg.package_name,
                                  )
                                }
                              >
                                ...
                                <TableCell>
                                  <div className="flex items-center gap-2">
                                    {pkg.downloaded ? (
                                      <span
                                        title="Package cached on central"
                ...
                {completedPackageCount > 0 && (
                  <p className="mt-3 text-xs text-zinc-500 text-center">
                    ...
                    {completedPackageCount} package
                    {completedPackageCount !== 1 ? "s" : ""} fully installed on
                    all nodes
                  </p>
                )}
              </>
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-2">
              ...
              {installPlan.rebootPkgs.length > 0 && (
                <div className="mb-4 rounded-md bg-amber-500/15 border border-amber-500/30 p-3 text-sm text-amber-300">
                  <AlertTriangle className="inline h-4 w-4 mr-1" />
                  {installPlan.rebootPkgs.join(", ")} require
                  {installPlan.rebootPkgs.length === 1 ? "s" : ""} a reboot
                  after installation. You can restart nodes after installation
                  from the System Nodes section.
                </div>
              )}
              ...
              <div className="mb-4 space-y-1">
                ...
                <Select
                  ...
                  onChange={(e) => setInstallStrategy(e.target.value)}
                  className="w-48"
                >
                  <option value="rolling">Rolling</option>
            ...
            <div className="p-6 pt-3 border-t border-zinc-800">
              {startInstallJob.isError && (
                <div className="mb-3 rounded-md bg-red-500/10 border border-red-500/30 p-2 text-xs text-red-400">
                  <AlertTriangle className="inline h-3 w-3 mr-1" />
                  {(startInstallJob.error as Error)?.message ??
                    "Failed to start install job"}
                </div>
              )}
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button
                  ...
                  disabled={
                    startInstallJob.isPending || installPlan.nodes.length === 0
                  }
                >
              ...
              {[...osInstallJobs]
                ...
                .map((job) => (
                  <OsInstallJobCard
                    ...
                    onToggle={() =>
                      setExpandedInstallJobId(
                        expandedInstallJobId === job.id ? null : job.id,
                      )
                    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <Badge className="bg-zinc-500/15 text-zinc-400 border-zinc-500/30 text-[10px]">
                {job.restart_policy === "none"
                  ? "no restart"
                  : job.restart_policy.replace(/_/g, " ") + " restart"}
              </Badge>
            </div>
            <div className="flex items-center gap-3 mt-0.5 text-xs text-zinc-500">
              {job.started_by && <span>by {job.started_by}</span>}
              {job.started_at ? (
                ...
                job.created_at && <span>created {timeAgo(job.created_at)}</span>
              )}
              {totalSteps > 0 && (
                <span>
                  {completedSteps}/{totalSteps} steps
