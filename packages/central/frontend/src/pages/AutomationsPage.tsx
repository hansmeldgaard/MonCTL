      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              ...
              value={target}
              onChange={(e) =>
                setTarget(e.target.value as "collector" | "central")
              }
            >
        ...
        <div className="space-y-2">
            ...
            {(["none", "type", "direct"] as const).map((opt) => (
              ...
              >
                <input
                  type="radio"
                  name="cred-mode"
                  checked={credMode === opt}
                  onChange={() => setCredMode(opt)}
          ...
          {credMode === "type" && (
            ...
            >
              ...
              {Array.isArray(credentialTypes) &&
                credentialTypes.map((ct: { id?: string; name: string }) => (
                ...
                credentials.map(
                  (c: {
                    ...
                    <option key={c.id} value={c.id}>
                      {c.name}
                      {c.credential_type ? ` (${c.credential_type})` : ""}
                    </option>
                  ),
                )}
            </Select>
          ...
          {credMode === "type" && (
            <p className="text-xs text-zinc-500">
              Resolved from the device's per-protocol credentials at runtime.
            </p>
          )}
          {credMode === "direct" && (
            <p className="text-xs text-zinc-500">
              Uses this specific credential directly (e.g. for central actions
        ...
        <div className="space-y-1.5">
          ...
          <div className="rounded-md border border-zinc-700 bg-zinc-950 text-xs text-zinc-500 px-3 py-2 mb-1 font-mono leading-relaxed">
            # context.device_id, context.device_name, context.device_ip
            <br />
            # context.credential — decrypted credential dict
            <br />
            # context.shared_data — data from previous steps
            <br /># context.set_output(key, value) — store data for next steps
          </div>
          <textarea
            id="action-code"
...
function AutomationsTab({
  ...
  const [page, setPage] = useState(1);
  const [createOpen, setCreateOpen] = useState(false);
  const [createPrefill, setCreatePrefill] = useState<AutomationPrefill | null>(
    null,
  );
...
function AutomationFormDialog({
  ...
  const [cooldown, setCooldown] = useState(
    String(automation?.cooldown_seconds ?? 300),
  );
  ...
  const [deviceScope, setDeviceScope] = useState<"all" | "devices" | "labels">(
    initDeviceIds.length
      ? "devices"
      : automation?.device_label_filter
        ? "labels"
        : "all",
  );
  const [selectedDeviceIds, setSelectedDeviceIds] =
    useState<string[]>(initDeviceIds);
  const [deviceSearch, setDeviceSearch] = useState("");
  const [labelKey, setLabelKey] = useState(
    Object.keys(automation?.device_label_filter ?? {})[0] ?? "",
  );
  const [labelValue, setLabelValue] = useState(
    Object.values(automation?.device_label_filter ?? {})[0] ?? "",
  );
  ...
  const [steps, setSteps] = useState<
    Array<{
      action_id: string;
      step_order: number;
      credential_type_override: string;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      device_ids:
        deviceScope === "devices" && selectedDeviceIds.length > 0
          ? selectedDeviceIds
          : null,
      device_label_filter:
        deviceScope === "labels" && labelKey
          ? { [labelKey]: labelValue }
          : null,
        ...
        <Tabs value={dialogTab} onChange={setDialogTab}>
          <TabsList>
            ...
            <TabTrigger value="actions">
              Actions {steps.length > 0 && `(${steps.length})`}
            </TabTrigger>
          </TabsList>
          ...
          <TabsContent value="general" className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="e.g. Auto-remediate link down"
                  autoFocus
                />
              </div>
              <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              />
              <p className="text-xs text-zinc-500">
                Minimum seconds between runs per device. 0 = no cooldown.
              </p>
            </div>
                ...
                {(["all", "devices", "labels"] as const).map((opt) => (
                  ...
                  >
                    <input
                      type="radio"
                      name="device-scope"
                      checked={deviceScope === opt}
                      onChange={() => setDeviceScope(opt)}
                      className="border-zinc-600"
                    />
                    {opt === "all"
                      ? "All devices"
                    ...
                    {filteredDevices.map((d) => (
                      ...
                      >
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked)
                              setSelectedDeviceIds((p) => [...p, d.id]);
                            else
                              setSelectedDeviceIds((p) =>
                                p.filter((id) => id !== d.id),
                              );
                          }}
                          className="rounded border-zinc-600"
                        ...
                        {labelKeys
                          ...
                          .map((lk) => (
                            <option key={lk.id} value={lk.key}>
                              {lk.key}
                              {lk.description ? ` — ${lk.description}` : ""}
                            </option>
                          ))}
                      </Select>
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs text-zinc-500 h-7 flex items-center">
                        Value
                      </p>
                      <Select
                        value={labelValue}
          ...
          <TabsContent value="actions" className="space-y-3">
            ...
            {steps.length === 0 && (
              <p className="text-xs text-zinc-500 py-2">
                No steps added. Steps are optional — automations can exist
                without actions.
              </p>
            )}
...
function DeleteAutomationDialog({
  ...
  return (
    <Dialog open onClose={onClose} title="Delete Automation">
      <p className="text-sm text-zinc-300">
        Delete <strong>{automation.name}</strong>? All steps will be removed.
    ...
    <Dialog open onClose={onClose} title={`Trigger: ${automation.name}`}>
      {result ? (
        <div className="space-y-3">
          ...
          <p className="text-xs text-zinc-500">
            Run IDs: {result.run_ids.join(", ").substring(0, 100)}...
          </p>
          <DialogFooter>
            <Button variant="secondary" onClick={onClose}>
              Close
        ...
        <div className="space-y-3">
          <p className="text-sm text-zinc-400">
            Select devices to run this automation on:
          </p>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={selectedDevices.length === 0 || triggerMut.isPending}
              className="gap-1.5"
            >
    ...
    <Dialog open onClose={onClose} title="Run Details" size="lg">
      {isLoading || !run ? (
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-0.5">
                {run.device_name} ({run.device_ip}) &middot; {run.trigger_type}{" "}
          ...
          {run.event_id && (
            <p className="text-xs text-zinc-500">
              Event: {run.event_severity} &mdash;{" "}
              {run.event_message.substring(0, 100)}
            </p>
          )}
            ...
            {(run.step_results ?? []).map((step: AutomationRunStepResult) => (
              ...
              >
                ...
                >
                  ...
                  <Badge
                    variant={step.target === "collector" ? "success" : "info"}
                    className="text-[10px]"
                  >
                    {step.target}
                  </Badge>
                  ...
                  <div className="border-t border-zinc-700 px-3 py-2 space-y-2">
                    {step.stdout && (
                      <div>
                        ...
                        <pre className="text-xs text-zinc-300 bg-zinc-950 rounded px-2 py-1 overflow-x-auto max-h-32 whitespace-pre-wrap">
                          {step.stdout}
                        </pre>
                      </div>
                    )}
                    {step.stderr && (
                      <div>
                        ...
                        <pre className="text-xs text-red-300 bg-zinc-950 rounded px-2 py-1 overflow-x-auto max-h-32 whitespace-pre-wrap">
                          {step.stderr}
                        </pre>
                      </div>
                    )}
                    {step.output_data &&
                      Object.keys(step.output_data).length > 0 && (
                        <div>
                          ...
                          <pre className="text-xs text-zinc-300 bg-zinc-950 rounded px-2 py-1 overflow-x-auto max-h-32">
