...
import {
  DashboardTimePicker,
  type TimeRange,
} from "@/components/DashboardTimePicker.tsx";
...
export function DashboardEditorPage() {
  ...
  const handleLayoutChange = useCallback(
    (layout: readonly LayoutItem[], _layouts: unknown) => {
      if (!mounted.current) {
  ...
  function updateWidget(
    idx: number,
    data: { title: string; config: AnalyticsWidgetConfig },
  ) {
    setWidgets((prev) =>
      prev.map((w, i) => (i === idx ? { ...w, ...data } : w)),
    );
    setDirty(true);
  ...
  function addVariableDef() {
    ...
    if (newVarDefault.trim()) {
      setVariableValues((prev) => ({
        ...prev,
        [newVar.name]: newVar.default_value,
      }));
    }
    setNewVarName("");
    setNewVarDefault("");
          ...
          {variableDefs.map((v) => (
            <div key={v.name} className="flex items-center gap-2 text-xs">
              ...
              <span className="text-zinc-500">
                default: {v.default_value || "(none)"}
              </span>
              <button
                onClick={() => removeVariableDef(v.name)}
